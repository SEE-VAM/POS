...
create or replace package body pkg_pos_util as
    ...
    begin
        return '₹ ' || trim(to_char(coalesce(p_amount, 0), '99,99,99,990.00'));
    end format_inr;
...
create or replace package body pkg_pos_invoice as
    ...
    begin
        ...
        -- Totals
        v_html := v_html || '<table style="width:100%; font-size: 11px;">';
        v_html := v_html || '<tr><td>Subtotal:</td><td style="text-align:right;">₹ ' || to_char(v_sale.subtotal, '99,990.00') || '</td></tr>';
        if v_sale.bill_discount_amount > 0 or v_sale.item_discount_amount > 0 then
            v_html := v_html || '<tr><td>Discount:</td><td style="text-align:right;">- ₹ ' || to_char(v_sale.bill_discount_amount + v_sale.item_discount_amount, '99,990.00') || '</td></tr>';
        end if;
        if v_sale.cgst_amount > 0 then
            v_html := v_html || '<tr><td>CGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.cgst_amount, '99,990.00') || '</td></tr>';
            v_html := v_html || '<tr><td>SGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.sgst_amount, '99,990.00') || '</td></tr>';
        end if;
        if v_sale.igst_amount > 0 then
            v_html := v_html || '<tr><td>IGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.igst_amount, '99,990.00') || '</td></tr>';
        end if;
        if v_sale.round_off_amount != 0 then
            v_html := v_html || '<tr><td>Round Off:</td><td style="text-align:right;">₹ ' || to_char(v_sale.round_off_amount, '90.00') || '</td></tr>';
        end if;
        v_html := v_html || '<tr style="font-size: 14px; font-weight: bold;"><td>GRAND TOTAL:</td><td style="text-align:right;">₹ ' || to_char(v_sale.grand_total, '99,990.00') || '</td></tr>';
        v_html := v_html || '</table><hr style="border-top: 1px dashed #000;"/>';

        -- Payments
        v_html := v_html || '<div style="font-size: 11px;"><b>Payments:</b></div>';
        for pay in (select * from pos_payments where sale_id = p_sale_id) loop
            v_html := v_html || '<div style="font-size: 10px; display:flex; justify-content:space-between;"><span>' || pay.payment_type || ':</span><span>₹ ' || to_char(pay.amount_received, '99,990.00') || '</span></div>';
        end loop;
    ...
    begin
        ...
        v_html := v_html || '<tr><td>Taxable Value:</td><td style="text-align:right;">₹ ' || to_char(v_sale.taxable_amount, '99,99,990.00') || '</td></tr>';
        if v_sale.cgst_amount > 0 then
            v_html := v_html || '<tr><td>CGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.cgst_amount, '99,99,990.00') || '</td></tr>';
            v_html := v_html || '<tr><td>SGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.sgst_amount, '99,99,990.00') || '</td></tr>';
        end if;
        if v_sale.igst_amount > 0 then
            v_html := v_html || '<tr><td>IGST:</td><td style="text-align:right;">₹ ' || to_char(v_sale.igst_amount, '99,99,990.00') || '</td></tr>';
        end if;
        if v_sale.round_off_amount != 0 then
            v_html := v_html || '<tr><td>Round Off:</td><td style="text-align:right;">₹ ' || to_char(v_sale.round_off_amount, '90.00') || '</td></tr>';
        end if;
        v_html := v_html || '<tr style="font-size:15px; font-weight:bold; background:#edf2f7;"><td>Invoice Total:</td><td style="text-align:right;">₹ ' || to_char(v_sale.grand_total, '99,99,990.00') || '</td></tr>';
